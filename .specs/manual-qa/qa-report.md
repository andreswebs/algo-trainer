...
**Severity:** High
**Expected:** `at init` in empty directory should initialize workspace there
**Actual:** Error: "Root directory cannot be empty"
**Exit Code:** 2
**Recommendation:** Allow `at init` without path argument to init in current directory
...
**Severity:** Low
**Expected:** `at list -t array` should filter to only problems with "array" tag
...
2. Allow `at init` to work in current directory without path argument
