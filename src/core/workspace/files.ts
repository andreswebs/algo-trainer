...
function validateRootDirectory(rootDir: string): void {
  ...
  // Check BEFORE normalizing to catch actual traversal attempts
  if (
    rootDir.includes('/../') || rootDir.includes('\\..\\') || rootDir.endsWith('/..') ||
    rootDir.endsWith('\\..')
  ) {
    throw new ValidationError(
...
function validateLanguage(language: SupportedLanguage): void {
  ...
  if (!validLanguages.includes(language)) {
    throw new ValidationError(
      ...
      createErrorContext('validateLanguage', {
        language,
        validLanguages: Array.from(validLanguages),
      }),
    );
  }
}
