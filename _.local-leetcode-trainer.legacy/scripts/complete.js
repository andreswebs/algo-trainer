...
const { getPracticePath } = require("./config.js");
...
function resolveProblemPath(input) {
  ...
  // Try to construct the full path: practice/difficulty/problem-name/problem-name.js
...
function markAsCompleted(problemInput) {
  ...
  // Handle practice/difficulty/problem structure
  const difficulty = parts[0] === "practice" ? parts[1] : parts[0];
  const problemName = parts[0] === "practice" ? parts[2] : parts[1];
  ...
  if (fs.existsSync(targetDir)) {
    console.log(
      `✅ ${difficulty}/${problemName} is already marked as completed!`
    );
    return false;
  }
...
function listCompleted() {
  ...
  ["easy", "medium", "hard"].forEach((difficulty) => {
    ...
    if (fs.existsSync(completedDir)) {
      ...
      if (problems.length > 0) {
        console.log(
          `📗 ${difficulty.toUpperCase()} (${problems.length} completed):`
        );
  ...
  if (totalCompleted === 0) {
    ...
    console.log(
      "  yarn test easy/problem-name  # Automatically finds in completed"
    );
    console.log("💡 To open a completed problem:");
    console.log(
      "  yarn open easy/problem-name  # Automatically finds in completed"
    );
  }
}
...
function markAsActive(problemInput) {
  ...
  if (!fs.existsSync(completedDir)) {
    console.log(
      `❌ Problem not found in completed: ${difficulty}/${problemName}`
    );
    return false;
  }
  ...
  if (fs.existsSync(activeDir)) {
    console.log(
      `❌ Problem already exists in active: ${difficulty}/${problemName}`
    );
    return false;
  }
...
function main() {
  ...
  if (!firstArg) {
    ...
    console.log("💡 Usage:");
    console.log(
      "  yarn complete easy/two-sum          # Mark problem as completed"
    );
    console.log(
      "  yarn complete list                  # List all completed problems"
    );
  ...
  if (
    firstArg.toLowerCase() === "undo" ||
    firstArg.toLowerCase() === "unmark"
  ) {
    const problemInput = args[1];
    if (!problemInput) {
      console.log(
        "❌ Please specify a problem: yarn complete undo easy/two-sum"
      );
      return;
    }
