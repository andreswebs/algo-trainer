...
const { DIFFICULTY_NAMES, getPracticePath } = require("./config.js");
const {
  getAllProblems,
  getStatistics,
} = require("./dynamic/problems/index.js");
...
function main() {
  ...
  const totalCompleted =
    completed.easy.length + completed.medium.length + completed.hard.length;
  ...
  ["easy", "medium", "hard"].forEach((difficulty) => {
    ...
    console.log(
      ...
      }: ${completedCount}/${availableCount} completed (${percentage}%) + ${activeCount} active`
    );
  });
  console.log();
  ...
  const filledLength = Math.round(
    (totalCompleted / stats.total) * progressBarLength
  );
  const progressBar =
