...
// Practice directory configuration
const PRACTICE_DIRECTORIES = [
  "practice/easy",
  "practice/medium",
  "practice/hard",
];
...
/**
 * Get the practice directory path for a given difficulty
 * @param {string} difficulty - The difficulty level (easy, medium, hard)
 * @returns {string} The full practice directory path
 */
function getPracticePath(difficulty) {
...
function archiveCurrentWork(currentLanguage) {
  ...
  ["easy", "medium", "hard"].forEach((difficulty) => {
    const sourcePath = path.join(projectRoot, getPracticePath(difficulty));
  ...
  console.log(
    `📦 Archived ${currentLanguage} work to archive-${currentLanguage}/`
  );
}
...
function showCurrentConfig() {
  ...
  console.log(
    `📊 Problems: ${totalProblems} total (${counts.easy} easy, ${counts.medium} medium, ${counts.hard} hard)`
  );
...
async function switchLanguage(newLanguage) {
  ...
  console.log(
    `❌ Archive all current problems to 'archive-${currentLanguage}/'`
  );
...
async function main() {
  ...
  if (!command) {
    ...
    console.log("💡 Usage:");
    console.log(
      "  yarn config                    # Show current configuration"
    );
