...
Common patterns: DFS (preorder, inorder, postorder) or BFS (level-order)`,
  ...
  addHashTableHints(script) {
    script.steps.push({
      type: "hint",
      trigger:
        'code.includes("for") && !code.includes("Map") && !code.includes("Set")',
...
Consider what you need to "remember" as you iterate!`,
  ...
  addTwoPointerHints(script) {
    script.steps.push({
      type: "hint",
      trigger:
        'code.includes("for") && !code.includes("left") && !code.includes("right")',
...
- How would you explain this solution in an interview?`,
  ...
  listTemplates() {
    ...
    console.log(
      "3. **advanced** - Sophisticated guidance for complex problems"
    );
    console.log("");
    console.log(
      "Usage: node generate-teaching-script.js <problem-path> [template-type]"
    );
  }
}
