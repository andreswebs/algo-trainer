...
class UniversalTeachingGenerator {
  ...
  async processProblemDirectory(problemPath, problemName, difficulty) {
    ...
    const success = await this.generateTeachingScript(
      problemPath,
      problemName,
      difficulty
    );
  ...
  async generateTeachingScript(problemPath, problemName, difficulty) {
    try {
      ...
      // Generate the script
      const scriptPath = this.generator.generateScript(
        problemPath,
        templateType,
        {
          language: "javascript",
  ...
  printSummary() {
    ...
    const coverage =
      this.stats.total > 0
        ? Math.round(
            ((this.stats.existing + this.stats.generated) / this.stats.total) *
              100
          )
        : 0;
    ...
    if (this.stats.errors > 0) {
      ...
      console.log(
        "Some problems failed to generate. Check the error messages above."
      );
    }
  }
}
