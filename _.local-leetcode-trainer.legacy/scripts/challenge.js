...
try {
  ...
  console.error(
    "  1. Reinstall the package: npm install -g local-leetcode-trainer@latest"
  );
  console.error("  2. Check if all files were installed correctly");
  console.error(
    "  3. Report this issue at: https://github.com/karote00/local-leetcode-trainer/issues"
  );
  console.error("");
...
if (
  !dynamicSystem ||
  !dynamicSystem.problemManager ||
  !dynamicSystem.offlineManager
) {
...
function getExistingProblems() {
  ...
  ["easy", "medium", "hard"].forEach((difficulty) => {
    const difficultyPath = path.join(projectRoot, getPracticePath(difficulty));
...
async function generateDynamicProblems(
  difficulty,
  count,
  ...
  if (specificProblem) {
    ...
    try {
      const problem = await dynamicSystem.problemManager.getProblem(
        specificProblem,
        {
      ...
      throw new Error(
        `Failed to fetch specific problem "${specificProblem}": ${error.message}`
      );
    }
  } else {
    ...
    for (let i = 0; i < count && attempts < maxAttempts; attempts++) {
      try {
        const problem = await dynamicSystem.problemManager.getRandomProblem(
          difficulty,
          {
            language: getCurrentLanguage(),
            includeHints: false,
        ...
        if (existing[difficulty].includes(problem.name)) {
          console.log(
            `⚠️  Problem already exists: ${problem.name}, trying another...`
          );
          continue;
        }
    ...
    if (problems.length === 0) {
      throw new Error(
        `Failed to fetch any ${difficulty} problems after ${maxAttempts} attempts`
      );
    }
  }
...
async function createProblemFiles(difficulty, problem) {
  try {
    ...
    const problemDir = path.join(getPracticePath(difficulty), problem.name);
    ...
    console.error(
      `❌ Failed to create files for ${problem.name}: ${error.message}`
    );
    return false;
  }
}
...
function displayProblemInfo(problem, created, index) {
  ...
  const sourceText =
    problem.metadata?.source === "cache" ? "Cache" : "LeetCode";
  ...
  const problemDir = path.join(
    projectRoot,
    getPracticePath(problem.difficulty),
    problem.name
  );
  const problemFile = path.join(
    problemDir,
    `${problem.name}${langConfig.extension}`
  );
  ...
  console.log(
    `   🏢 ${
      (problem.companies || []).slice(0, 3).join(", ") || "No companies"
    }`
  );
  if (created) {
...
async function showProgressStats() {
  ...
  ["easy", "medium", "hard"].forEach((difficulty) => {
    const activePath = path.join(projectRoot, getPracticePath(difficulty));
    const completedPath = path.join(
      projectRoot,
      getPracticePath(difficulty),
      "completed"
    );
    ...
    console.log(
      ...
      }: ${total} total (${activeCount} active, ${completedCount} completed)`
    );
  });
  ...
  try {
    const offlineProblems =
      await dynamicSystem.offlineManager.getOfflineProblemList();
    console.log(`\n📱 Offline Problems Available: ${offlineProblems.count}`);

    if (offlineProblems.count > 0) {
      const grouped = {
        easy: offlineProblems.problems.filter((p) => p.difficulty === "easy")
          .length,
        medium: offlineProblems.problems.filter(
          (p) => p.difficulty === "medium"
        ).length,
        hard: offlineProblems.problems.filter((p) => p.difficulty === "hard")
          .length,
      };
      console.log(
        `  Easy: ${grouped.easy}, Medium: ${grouped.medium}, Hard: ${grouped.hard}`
      );
    }
  } catch (error) {
...
async function main() {
  ...
  const words = args.filter(
    (arg) =>
      !DIFFICULTY_NAMES.includes(arg.toLowerCase()) && !arg.startsWith("--")
  );
  ...
  if (!difficulty && !specificProblem) {
    ...
    console.log("💡 Usage:");
    console.log(
      "  lct challenge easy              # Get 1 random easy problem from LeetCode"
    );
    console.log(
      "  lct challenge medium 2          # Get 2 random medium problems from LeetCode"
    );
    console.log(
      "  lct challenge hard              # Get 1 random hard problem from LeetCode"
    );
    console.log(
      "  lct challenge two-sum           # Get specific problem by name"
    );
    console.log(
      "  lct challenge 3sum              # Get specific problem by name"
    );
    console.log("  lct challenge 1                 # Get problem by ID");
    console.log("");
    console.log(
      "🌐 All problems are fetched directly from LeetCode in real-time!"
    );
    console.log("");
    ...
    console.log(
      "  lct hint <problem> <level>      # Get AI hints (levels 1-5)"
    );
  ...
  try {
    ...
    const selectedProblems = await generateDynamicProblems(
      difficulty,
      count,
      specificProblem
    );
    ...
    console.log(
      `🎯 Generated ${selectedProblems.length} ${difficulty || "problem"}${
    ...
    if (!connectivity.isOnline) {
      ...
      console.log(
        "  lct cache list                  # See available offline problems"
      );
      console.log(
        "  lct cache prepare               # Cache problems when online"
      );
    } else {
      console.log("");
