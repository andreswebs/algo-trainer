...
function runJavaScriptTests(problemPath) {
  try {
    ...
    console.log(
      "💡 Make sure your solution file exports the function correctly."
    );
  }
}
...
function runJavaTests(problemPath) {
  ...
  exec(
    `cd "${dir}" && javac ${fileName}.java && java ${fileName}`,
    (error, stdout, stderr) => {
      if (error) {
...
function runCppTests(problemPath) {
  ...
  // Compile and run C++
  exec(
    `cd "${dir}" && g++ -o ${fileName} ${fileName}.cpp && ./${fileName}`,
    (error, stdout, stderr) => {
      if (error) {
...
function resolveProblemPath(input) {
  ...
  // Try to construct the full path: practice/difficulty/problem-name/problem-name.ext
  ...
  if (parts.length === 2) {
    ...
    const activePath = path.join(
      ...
      problemName,
      `${problemName}${langConfig.extension}`
    );
