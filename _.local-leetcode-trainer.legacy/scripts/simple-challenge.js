...
class SimpleChallengeGenerator {
  ...
  async generateChallenge(difficulty, count = 1) {
    try {
      ...
      for (let i = 0; i < count; i++) {
        const problemSlug = this.selectRandomProblem(
          difficulty,
          existing[difficulty]
        );
  ...
  selectRandomProblem(difficulty, existingProblems) {
    ...
    const unsolvedProblems = availableProblems.filter(
      (problem) => !existingProblems.includes(problem)
    );
  ...
  async createProblemFiles(difficulty, problemData) {
    try {
      ...
      const problemFile = this.generateProblemFile(
        problemData,
        language,
        langConfig
      );
...
module.exports = solution;`;
  ...
  generateTestFile(problemData, language, langConfig) {
    if (language === "javascript") {
      ...
      const testCaseArray = problemData.examples
        .map((ex, i) => {
          return `  {
    ...
    category: "basic"
      ...
      return `const ${problemData.slug.replace(/-/g, "")} = require('./${
        problemData.slug
      }');
...
- Use \`yarn lct test ${problemData.difficulty}/${
      problemData.slug
    }\` to run tests
- Use \`yarn lct learn ${problemData.difficulty}/${
      problemData.slug
    }\` for AI help
`;
  }
  ...
  displayResults(results) {
    ...
    results.forEach(({ problem, created, index }) => {
      ...
      if (created) {
        console.log(
          `   📁 Created in: ${getPracticePath(problem.difficulty)}/${
            problem.slug
          }/`
        );
      }
...
async function main() {
  ...
  if (!difficulty) {
    ...
    console.log("Usage:");
    console.log(
      "  node scripts/simple-challenge.js easy 1     # Generate 1 easy problem"
    );
    console.log(
      "  node scripts/simple-challenge.js medium 2   # Generate 2 medium problems"
    );
    console.log(
      "  node scripts/simple-challenge.js hard       # Generate 1 hard problem"
    );
    console.log("");
